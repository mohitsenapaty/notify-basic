...
const TicketNotification = require('../api/models/ticketnotification.model');
...
const app = Consumer.create({
  ...
  handleMessage: async (message) => {
    ...
    logger.info(`consumed notification with body : ${JSON.stringify(body)}`);
    const notification = await TicketNotification.findOne({
      _id: msg.id,
      status: statusMap.created,
    }).populate('category provider template type');

    logger.info(`notification retrieved : ${JSON.stringify(notification)}`);
    if (notification) {
